"""Transformer decoder models for DNA sequence reconstruction from embeddings."""
...
class DecoderReconstructor(nn.Module):
    """Unified transformer decoder-based DNA sequence reconstructor for both mean and per-nucleotide embeddings.
    ...
    3. Transformer decoder blocks (with causal masking)
    ...
    d_model : int
        The dimension of the transformer model. Must be positive.
    nhead : int
        Number of attention heads. Must divide d_model evenly.
    num_layers : int
        Number of transformer decoder layers. Must be positive.
    dim_feedforward : int
        Dimension of the feedforward network in transformer. Must be positive.
    dropout : float
    ...
    def __init__(
        ...
        if mode == "mean":
            ...
            logger.info(
                f"Decoder [{mode}]: input_dim={input_dim} -> "
        ...
        causal_mask = self._generate_square_subsequent_mask(seq_length)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.transformer_decoder(tgt=x, memory=x, tgt_mask=self.causal_mask)
