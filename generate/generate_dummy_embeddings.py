...
This script creates random DNA sequences with per-nucleotide embeddings.
...
Example usage:
    python generate_dummy_embeddings.py --config-name generate_data num_sequences=1000 seq_length=50 embedding_dim=128
    python generate_dummy_embeddings.py num_sequences=500 train_split=0.7 val_split=0.15
"""
...
def generate_embedding(
    ...
    """Generate a per-nucleotide embedding for a DNA sequence with noise.
    ...
    signal_strength : float
        Strength of the nucleotide signal in the first 4 dimensions (0-1).

    Returns
    -------
    np.ndarray
        Per-nucleotide embedding of shape (len(sequence), embedding_dim).
        Each nucleotide gets a weak signal in the first 4 dimensions plus noise,
        with remaining dimensions filled with pure noise.
    """
    assert embedding_dim >= len(NUCLEOTIDES), f"embedding_dim must be at least {len(NUCLEOTIDES)}"
    ...
    # Add weak nucleotide signals to the first 5 dimensions
    for i, nuc in enumerate(sequence.upper()):
        ...
        # Add signal strength to the appropriate nucleotide position
        embeddings[i, idx] += signal_strength
...
def main(cfg: DictConfig) -> None:
    ...
    logger.info("Generating synthetic DNA sequences and embeddings...")
    logger.info(
