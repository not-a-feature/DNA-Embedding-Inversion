...
def evaluate_single_run(
    ...
    # --- 1000g Patch ---
    logger.info(f"Patching data config for 1000g evaluation...")
    original_test = data_cfg.test_csv
    if "hg38" in original_test:
        patched = original_test.replace("hg38", "1000g")
    else:
        # Fallback if hg38 is not in the string but we know it's a 1000g eval
        patched = original_test.replace(".h5", "_1000g.h5")

    # Normalise evo2 variant names: 1000g files use "evo2" (not "evo2_deep")
    patched = patched.replace("evo2_deep", "evo2")

    data_cfg.test_csv = patched
    data_cfg.skip_sha256_check = True
    logger.info(f"Changed test_csv from {original_test} to {data_cfg.test_csv}")
    
    if not os.path.exists(hydra.utils.to_absolute_path(data_cfg.test_csv)):
        logger.warning(f"1000g test set not found at {data_cfg.test_csv}, skipping this run.")
        return None
    # -------------------
...
@hydra.main(config_path="conf", config_name="evaluate_1000g", version_base=None)
def main(cfg: DictConfig) -> None:
    ...
    if cfg.only_plots:
        ...
        for i, subdir in enumerate(subdirs):
            ...
            # Evaluate this run (skip gracefully on errors)
            try:
                result = evaluate_single_run(subdir, device, run_output_dir, logger, cfg.max_samples)
            except Exception as e:
                logger.warning(f"Skipping run {job_num} due to error: {e}")
                continue

            if result is None:
                logger.info(f"Skipped run {job_num} (no 1000g data for this seq length)")
                continue
